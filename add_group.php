...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','<b>Maaf!</b> Nama Grup sudah ada di database!');
     redirect('add_group.php', false);
   }elseif(find_by_groupLevel($_POST['group-level']) === false) {
     $session->msg('d','<b>Maaf!</b> Level Grup sudah ada di database!');
     redirect('add_group.php', false);
   }
   if(empty($errors)){
        ...
        if($db->query($query)){
          //sucess
          $session->msg('s',"Grup berhasil dibuat! ");
          redirect('add_group.php', false);
        } else {
          //failed
          $session->msg('d',' Maaf gagal membuat grup!');
          redirect('add_group.php', false);
        }
   } else {
