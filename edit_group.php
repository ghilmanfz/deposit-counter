...
<?php
  ...
  if(!$e_group){
    $session->msg("d","ID Grup tidak ditemukan.");
    redirect('group.php');
  }
?>
<?php
   ...
   if(empty($errors)){
        ...
        $result = $db->query($query);
         if($result && $db->affected_rows() === 1){
          //sucess
          $session->msg('s',"Grup berhasil diupdate! ");
          redirect('edit_group.php?id='.(int)$e_group['id'], false);
        } else {
          //failed
          $session->msg('d',' Maaf gagal mengupdate grup!');
          redirect('edit_group.php?id='.(int)$e_group['id'], false);
        }
   } else {
        ...
        <div class="form-group">
          <label for="status">Status</label>
              <select class="form-control" name="status">
                <option <?php if($e_group['group_status'] === '1') echo 'selected="selected"';?> value="1"> Aktif </option>
                <option <?php if($e_group['group_status'] === '0') echo 'selected="selected"';?> value="0">Nonaktif</option>
              </select>
        </div>
